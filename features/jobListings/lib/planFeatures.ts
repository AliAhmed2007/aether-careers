...
export async function hasReachedMaxPublishedJobListings() {
    ...
    const canPost = await Promise.all([
        ...
        hasPlanFeature("post_10_job_listing").then(has => has && count < 10),
    ])
...
export async function hasReachedMaxFeaturedJobListings() {
    ...
    const canFeature = await Promise.all([
        hasPlanFeature("add_1_feature_job_listing").then(has => has && count < 1),
        hasPlanFeature("add_unlimited_featured_job_listings"),
    ])
